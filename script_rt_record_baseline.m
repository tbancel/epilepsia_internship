...
number_of_minutes = 3; % time to record

approx_epoch_timelength = 0.2; % 200ms
recording_time = number_of_minutes*60; % in seconds
...
f=figure(1)
f.Name = strcat(timestr,'_baseline_recording_figure');
plot(realtime, realdata);
xlabel("Time (s)");
title(strcat("Channel CED Analysis: ", num2str(chan)));
saveas(f,strcat(f.Name, '.png'), 'png');
...
% filter raw signal (along the first dimension)

% /!\ WE DON'T FILTER THE SIGNAL IN REALTIME

% f_c = [1 45];
% [b, a] = butter(5, 2*f_c*dt, 'pass');
% fdata = filtfilt(b, a, data')'; % operate along the first dimension
% realfdata = reshape(fdata', [1 numel(data)]);

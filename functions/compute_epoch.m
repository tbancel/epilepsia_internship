function output = compute_epoch(signal, epoch_length, dt)
    % INPUTS:
    % - signal
    % - epoch length (P) number of points in each epochs
    % - dt : timelaps between each point
    % 
    % OUTPUT:
    ...
    output.chunked_time_vector = (1:numel(epoched_signal))*dt;
    
    output.epoch_timestamps = (((0:(number_of_epochs-1))*epoch_length+1)*dt)'; % <number_of_epochs> x 1 matrix

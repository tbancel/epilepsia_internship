...
recording_time = 0.2*60; % in seconds
...
for i=1:number_of_loops
    ...
    if i>1
        ...
        if min(prev_epoch) < threshold_value &&  max(prev_epoch) > threshold_value
           seizures(i-1,1) = 1;
           Master8.Trigger(3);
        ...
        disp(strcat("Time to compute epoch: ", num2str(time_to_compute(i,1))));
        drawnow;
    end

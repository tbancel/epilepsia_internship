function output = find_waves_and_stimulate(fs, sampled_time, sampled_data, Master8, seizures, stim_params, d_wave_timestamps)
...
function sendStimulation(mTimer, ~)
	...
	if abs(min(toc - executed_stimulation_times)) < 0.100
		Master8.Trigger(3);
		exec_time = toc;
